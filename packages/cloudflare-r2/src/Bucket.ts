...
import { createReadStream, type PathLike, type ReadStream } from "fs";
...
export class Bucket {
	...
	public getPublicUrl(): string | undefined {
		return this.bucketPublicUrls.length
			? this.bucketPublicUrls.at(0)
			: undefined;
	}
	 ...
	 * @param expiresIn Expiration time in seconds.
	 */
	public async putObjectSignedUrl(
		objectKey: string,
		expiresIn: number,
	): Promise<string> {
	...
	protected generateObjectPublicUrls(objectKey: string): Array<string> {
		...
		return this.bucketPublicUrls.map(
	...
	public getObjectPublicUrls(objectKey: string): string[] {
		return this.bucketPublicUrls.map(
			(bucketPublicUrl) => `${bucketPublicUrl}/${objectKey}`,
		);
	}
