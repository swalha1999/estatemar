...
function PropertyCard({ listing, showPopularLabel = false, lng }: PropertyCardProps) {
    const { t } = useTranslation(lng, "landing-page", {});

    const parsedImages =
        typeof listing.images === "string" ? JSON.parse(listing.images) : listing.images;

    const firstImage = Array.isArray(parsedImages) ? parsedImages[0]?.image?.url : undefined;
            ...
            <div className="most-popular-item">
                    ...
                    <div className="badge-container">
                        {listing.is_best_seller && (
                            <span className="card-badge best-seller-badge">
                                <Icon name="FaFire" className="badge-icon" /> {t("listingTable.bestSellers")}
                            </span>
                        )}
                        {listing.is_recommended && (
                            <span className="card-badge recommended-badge">
                                <Icon name="FaAward" className="badge-icon" /> {t("listingTable.recommended")}
                            </span>
                        )}
                    </div>
                    {firstImage && (
                ...
                <div className="units-status-container">
                    <div className="units-status">
                        ...
                        <span className="units-label">{t("listingTable.unitsAvailable")}</span>
                    </div>
                </div>
