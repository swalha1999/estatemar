...
import { db } from "@/db";
import { articles, files, type Article, type File as DBFile } from "@/db/schema";
...
export async function getArticleBySlug(slug: string, lng: string): Promise<ArticleWithImage> {
    const article = await db
        .select({
            article: articles,
            image: files,
...
export async function getPublishedArticles(lng: string): Promise<ArticleWithImage[]> {
    const articlesFromDb = await db
        .select({
            article: articles,
            image: files,
